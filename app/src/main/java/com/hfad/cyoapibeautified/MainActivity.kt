...
import com.hfad.cyoapibeautified.PokemonAdapter
import okhttp3.*
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    val weight = jsonObject.getInt("weight").toString() // Convert weight to a string
                    val height = jsonObject.getInt("height").toString()

                    runOnUiThread {
                        // Set a default description or leave it empty
                        val newPokemon = Pokemon(name, imageUrl, weight, height)
